...
return [
     ...
     */
    'providers' => [
        // Оставлен как есть (пакет требует наличия провайдера 'default').
        // Реально в проекте не используется — Detailing CRM работает через
        // three проекта-специфичных провайдера ниже: local_default / local_coder / architect.
        'default' => [
            'label' => 'openai',
        ...
        /**
         * ==============================================================
         * ПРОЕКТ-СПЕЦИФИЧНЫЕ ПРОВАЙДЕРЫ (Детейлинг CRM, раздел 16 системной
         * инструкции). Именно эти три провайдера использует CrmDevAgent.
         * ==============================================================
         */

        /**
         * Основной провайдер локального агента-исполнителя.
         * Qwen3 8B на удалённой Windows 11 машине (Ollama), доступ по Tailscale.
         * Общие задачи, диалог по спецификации, генерация PHP/Vue кода.
         */
        'local_default' => [
            'label' => 'ollama',
            'driver' => OllamaDriver::class,
            // Явно переопределяем api_url — по умолчанию OllamaDriver ждёт
            // http://localhost:11434/v1, что неверно для удалённой машины.
            'api_url' => env('OLLAMA_ENDPOINT', 'http://100.108.81.27:11434/v1'),
            'model' => 'qwen3:8b',
            // 30-50% от реального контекстного окна модели (раздел про
            // truncation_threshold ниже в файле) — для Qwen3 8B консервативно
            // берём ~16k при native-контексте ~32k.
            'default_truncation_threshold' => 16000,
            'default_max_completion_tokens' => 4000,
            'default_temperature' => 0.2, // низкая — кодогенерация, не творческая задача
        ],

        /**
         * Второй провайдер локального агента — узкая кодогенерация без
         * необходимости долгого диалогового контекста.
         * Qwen2.5-Coder 7B на той же Windows-машине.
         */
        'local_coder' => [
            'label' => 'ollama',
            'driver' => OllamaDriver::class,
        ...
        /**
         * Провайдер-архитектор — облачный API (раздел 16.8 системной
         * инструкции). Формулирует спецификации задач и ревьюит диффы
         * перед PR. Низкая частота вызовов оправдывает облачную модель.
         */
        'architect' => [
            'label' => 'claude',
            'api_key' => env('ANTHROPIC_API_KEY'),
        ...
        /**
         * Оставлен для справки/совместимости с шаблоном пакета. Не используется
         * проектом напрямую — используй 'local_default'/'local_coder' выше,
         * которые явно указывают на удалённую Windows-машину через Tailscale.
         */
        'ollama' => [
            'label' => 'ollama',
            'driver' => OllamaDriver::class,
