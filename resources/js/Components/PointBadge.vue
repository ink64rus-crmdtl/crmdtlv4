...
// Бейджи Точка/Юрлицо для карточек записей (WorkOrder, Transaction и т.п.).
// Показываются по тому же принципу, что и переключатели в топбаре
// (AppHeader.vue) — только если у тенанта реально больше одной точки
...
<template>
    <span v-if="showBranch && branch" class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300" :title="branch.deleted_at ? 'Точка удалена из системы' : 'Точка'">
