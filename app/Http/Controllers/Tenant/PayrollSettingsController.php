...
class PayrollSettingsController extends Controller
{
    ...
    private function assertNoDuplicate(array $data, ?int $excludeRuleId = null): void
    {
        ...
        if ($query->exists()) {
            throw ValidationException::withMessages([
                'target' => 'Такая ставка уже настроена для этой должности/сотрудника на эту услугу (группу/по умолчанию) и точку. Отредактируйте существующую запись вместо создания дубликата.',
            ]);
        }
    }
}
