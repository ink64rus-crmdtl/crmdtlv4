    ...
    public function convertToWorkOrder(Appointment $appointment)
    {
        ...
        $workOrder = DB::transaction(function () use ($appointment) {
            $workOrder = WorkOrder::create([
                ...
                // Дата/время создания заказа — момент конвертации (CLAUDE.md "Дата/время
                // создания и готовности заказ-наряда"), а не время начала записи. Дедлайн
                // "выполнить до" — берётся из записи (её конец), т.к. запись и есть
                // "связанная запись в календаре", о которой идёт речь в требовании.
                // Голый now() (не через TimezoneResolver) — момент во времени не зависит
                // от часового пояса просмотра, а WorkOrder.order_date хранится в UTC:
                // now($branchTz) без явного ->utc() отдал бы Eloquent'у московские
                // "числа на часах", которые тот записал бы в UTC-колонку как есть,
                // сдвинув реальное время на разницу поясов (живой баг, пойманный на
                // реальном тенанте при проверке).
                'order_date' => now(),
                'ready_at' => $appointment->end_at,
                'total_amount' => 0,
                'discount_amount' => 0,
                'final_amount' => 0,
            ]);
